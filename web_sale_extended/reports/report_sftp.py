...
class SftpReportLine(models.Model):
    ...
    def _cron_generate_ap_sftp_report(self):        
        ...
        records_ap_cancellation =  self.env['report.sftp.cancellation'].search([('send_sftp_cancellation_ok', '=', False)])
        records_bef_cancellation =  self.env['report.beneficiary.sftp.cancellation'].search([('send_sftp_cancellation_ok', '=', False)])
        ...
        for record in records_ap_cancellation:
            data.append([
                record.policy_number if record.policy_number != False else '', 
                record.certificate_number if record.certificate_number != False else '', 
                record.firstname if record.firstname != False else '', 
        ...
        for record in records_bef:
            ...
            for record in records_bef_cancellation:
                data2.append([
                    record.policy_number if record.policy_number != False else '', 
                    record.certificate_number if record.certificate_number != False else '', 
                    record.firstname if record.firstname != False else '', 
        ...
        if len(data) != 0:
            ...
            if sftp_server_env:
                ...
                try: 
                    ...
                    for record in records_ap_cancellation:
                        record.partner_id.write({
                            'send_sftp_cancellation_ok': True
                        })
                    for record in records_bef:
                        record.partner_id.write({
                            'send_sftp_ok': True
